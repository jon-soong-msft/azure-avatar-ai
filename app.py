...
# Create the SocketIO instance with proper configuration
...
DEFAULT_AZURE_OPENAI_DEPLOYMENT_NAME = 'gpt-4'

# Get configuration from environment variables
SPEECH_REGION = os.environ.get('SPEECH_REGION', DEFAULT_SPEECH_REGION)
SPEECH_KEY = os.environ.get('SPEECH_KEY')  # Required
AZURE_OPENAI_ENDPOINT = os.environ.get('AZURE_OPENAI_ENDPOINT')  # Required for chat
AZURE_OPENAI_API_KEY = os.environ.get('AZURE_OPENAI_API_KEY')  # Required for chat
AZURE_OPENAI_DEPLOYMENT_NAME = os.environ.get('AZURE_OPENAI_DEPLOYMENT_NAME', DEFAULT_AZURE_OPENAI_DEPLOYMENT_NAME)
COGNITIVE_SEARCH_ENDPOINT = os.environ.get('COGNITIVE_SEARCH_ENDPOINT')  # Optional
COGNITIVE_SEARCH_API_KEY = os.environ.get('COGNITIVE_SEARCH_API_KEY')  # Optional
COGNITIVE_SEARCH_INDEX_NAME = os.environ.get('COGNITIVE_SEARCH_INDEX_NAME')  # Optional
...
speech_resource_url = None  # e.g. /subscriptions/6e83d8b7-00dd-4b0a-9e98-dab9f060418b/resourceGroups/my-rg/providers/Microsoft.CognitiveServices/accounts/my-speech (optional)
user_assigned_managed_identity_client_id = None  # e.g. the client id of user assigned managed identity (optional)
...
@app.route("/health")
def health():
    return {"status": "healthy", "service": "myATO-AI-Chatbot"}, 200
...
def getIceToken() -> Response:
    ...
    # Wait for ICE token to be initialized
    while not ice_token:
        time.sleep(0.2)
...
def connectAvatar() -> Response:
    ...
    try:
        ...
        # Build talkingAvatar config based on whether it's customized or prebuilt
        # Capitalize character name (e.g., 'lisa' -> 'Lisa')
        avatar_character_capitalized = avatar_character.capitalize() if avatar_character else avatar_character
        
        if is_custom_avatar.lower() == 'true':
            # Customized avatar: combine character and style
            talking_avatar_config = {
                'customized': True,
                'character': f'{avatar_character_capitalized}-{avatar_style}',
                'background': {
            ...
            # Prebuilt avatar: separate character and style
            talking_avatar_config = {
                'customized': False,
                'character': avatar_character_capitalized,
                'style': avatar_style,
                'background': {
                    'color': '#00FF00FF' if transparent_background.lower() == 'true' else background_color,
                    'image': {
        ...
        avatar_config = {
            'synthesis': {
                'video': {
                    ...
                    'talkingAvatar': talking_avatar_config
                }
            }
        }
        
        # Debug logging
        print(f'Avatar configuration being sent:')
        print(f'  Character: {talking_avatar_config.get("character")}')
        print(f'  Style: {talking_avatar_config.get("style")}')
        print(f'  Customized: {talking_avatar_config.get("customized")}')
        print(f'Full config: {json.dumps(avatar_config, indent=2)}')
        ...
        print(f"Error connecting to avatar service: {e}")
        return Response(f"Error connecting to avatar service: {e}", status=500)
        ...
        def stt_recognized_cb(evt):
            if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
                try:
                    ...
                    # Emit user message to client so it can display it as a separate bubble
                    print(f'Emitting user message: {user_query} to client {client_id}')
                    socketio.emit("response", { 'path': 'api.userMessage', 'userMessage': user_query, 'clientId': str(client_id) })
                    print(f'User message broadcasted')
                    ...
                    socketio.emit("response", { 'path': 'api.chat', 'chatResponse': f"<STTL>{stt_latency}</STTL>", 'clientId': str(client_id) })
                    ...
                    for chat_response in handleUserQuery(user_query, client_id):
                        # No need to send "Assistant:" prefix since messages are displayed as separate bubbles
                        socketio.emit("response", { 'path': 'api.chat', 'chatResponse': chat_response, 'clientId': str(client_id) })
                except Exception as e:
